...
void WokerManager::Add_Emp() {
	...
	if (addNum > 0) {
		...
		for (int i = 0; i < addNum; i++) {
			...
			{
			case 1:
				workerbase = new Employee(id, name, 1);
				break;
			case 2:
				workerbase = new Manager(id, name, 2);
				break;
			case 3:
				workerbase = new Boss(id, name, 3);
				break;
			default:
				break;
			}
...
void WokerManager::Mod_Emp() {
	if (this->m_FilesIsEmpty) {
		...
		if (ret != -1) {
			...
			{
			case 1:
				wokerbase = new Employee(newId, newName, dSelect);
			case 2:
				wokerbase = new Manager(newId, newName, dSelect);
			case 3:
				wokerbase = new Boss(newId, newName, dSelect);
			}
			//数据更新到数组中
